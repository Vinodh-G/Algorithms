...
func isRoomsAvailable(arrivals: [Int], departures: [Int], numOfRooms: Int) -> Bool {
    guard numOfRooms > 0 else { return false }
    guard arrivals.count == departures.count else { return false }

    let arrivals = arrivals.sorted()
    let departures = departures.sorted()
    
    var startIndex = 0
    var endIndex = 0
    var maxNumOfRums = 0
    var currentNumOfRooms = 0
    
    while startIndex < arrivals.count, endIndex < departures.count {
        if arrivals[startIndex] < departures[endIndex] {
            startIndex += 1
            currentNumOfRooms += 1
            maxNumOfRums = max(maxNumOfRums, currentNumOfRooms)
        } else {
            endIndex += 1
            currentNumOfRooms -= 1
        }
    ...
    return maxNumOfRums == numOfRooms
}


print(isRoomsAvailable(arrivals: [1, 3, 5],
                       departures: [2, 6, 8], numOfRooms: 1))



print(isRoomsAvailable(arrivals: [1, 1, 2, 4],
                       departures: [3, 3, 5, 6], numOfRooms: 3))
